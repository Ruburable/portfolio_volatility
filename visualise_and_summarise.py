...
function populateProposals(){{const c=document.getElementById('proposals-content');if(!c.innerHTML){{let h='';const opTitles={{'1_operations':'1 Operation (Single Buy)','2_operations':'2 Operations (Two Buys)','3_operations':'3 Operations (Three Buys)','4_operations':'4 Operations (Four Buys)','5_operations':'5 Operations (Five Buys)'}};for(const[opKey,ports]of Object.entries(portfoliosData)){{if(ports.length>0){{h+=`<div class="category-section"><h2 class="category-title">${{opTitles[opKey]}}</h2><p style="color:#aaa;margin-bottom:20px">Sorted by Sharpe Ratio (Best to Worst) - Budget: 10% of Portfolio</p><div class="portfolio-grid">`;ports.forEach((p,i)=>{{const rankBadge=i<3?`<span style="background:#52B788;color:#0a0a0a;padding:3px 8px;border-radius:5px;font-size:0.8em;font-weight:bold;margin-left:10px">Top ${{i+1}}</span>`:'';h+=`<div class="portfolio-card" onclick="selectPortfolio('${{opKey}}',${{i}})"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px"><span style="font-size:1.1em;font-weight:500">Portfolio #${{i+1}}</span>${{rankBadge}}</div><div class="portfolio-metric"><span class="metric-label">Return:</span><span class="metric-value">${{(p.returns*100).toFixed(2)}}%</span><span class="${{p.return_improvement>=0?'improvement-positive':'improvement-negative'}}"> (${{p.return_improvement>=0?'+':''}}${{p.return_improvement.toFixed(2)}}%)</span></div><div class="portfolio-metric"><span class="metric-label">Volatility:</span><span class="metric-value">${{(p.volatility*100).toFixed(2)}}%</span><span class="${{p.volatility_change<0?'improvement-positive':'improvement-negative'}}"> (${{p.volatility_change.toFixed(2)}}%)</span></div><div class="portfolio-metric"><span class="metric-label">Sharpe:</span><span class="metric-value">${{p.sharpe_ratio.toFixed(4)}}</span><span class="improvement-positive"> (+${{p.sharpe_improvement.toFixed(2)}}%)</span></div><div style="margin-top:15px;padding-top:15px;border-top:1px solid #333"><div style="color:#52B788;font-size:0.85em;font-weight:500;margin-bottom:5px">Required Actions:</div><div style="color:#aaa;font-size:0.85em;line-height:1.6">${{p.trades}}</div></div></div>`}});h+=`</div></div>`}}}}c.innerHTML=h||'<p style="text-align:center;color:#aaa;padding:50px">No improvements found</p>'}}}}
function selectPortfolio(opKey,idx){{const p=portfoliosData[opKey][idx];document.querySelectorAll('.portfolio-card').forEach(c=>c.classList.remove('selected'));event.currentTarget.classList.add('selected');const rDiff=p.returns-currentPortfolio.return;const vDiff=p.volatility-currentPortfolio.volatility;const sDiff=p.sharpe_ratio-currentPortfolio.sharpe;const opNum=parseInt(opKey.split('_')[0]);document.getElementById('comparison-content').innerHTML=`<div class="comparison-section"><h3 style="color:#52B788;margin-bottom:5px">Portfolio Comparison</h3><p style="color:#aaa;font-size:0.9em;margin-bottom:15px">${{opNum}} Operation${{opNum>1?'s':''}} Required</p><div class="comparison-row"><span>Return</span><div><div style="color:#FF6B6B">${{(currentPortfolio.return*100).toFixed(2)}}%</div><div style="color:#52B788">${{(p.returns*100).toFixed(2)}}%</div></div></div><div class="comparison-row"><span>Volatility</span><div><div style="color:#FF6B6B">${{(currentPortfolio.volatility*100).toFixed(2)}}%</div><div style="color:#52B788">${{(p.volatility*100).toFixed(2)}}%</div></div></div><div class="comparison-row"><span>Sharpe</span><div><div style="color:#FF6B6B">${{currentPortfolio.sharpe.toFixed(4)}}</div><div style="color:#52B788">${{p.sharpe_ratio.toFixed(4)}}</div></div></div></div><div class="comparison-section"><h3 style="color:#52B788">Improvements</h3><div class="comparison-row"><span>Return</span><span class="${{rDiff>=0?'improvement-positive':'improvement-negative'}}">${{rDiff>=0?'+':''}}${{(rDiff*100).toFixed(2)}}%</span></div><div class="comparison-row"><span>Volatility</span><span class="${{vDiff<=0?'improvement-positive':'improvement-negative'}}">${{vDiff>=0?'+':''}}${{(vDiff*100).toFixed(2)}}%</span></div><div class="comparison-row"><span>Sharpe</span><span class="${{sDiff>=0?'improvement-positive':'improvement-negative'}}">${{sDiff>=0?'+':''}}${{sDiff.toFixed(4)}} (${{p.sharpe_improvement>=0?'+':''}}${{p.sharpe_improvement.toFixed(2)}}%)</span></div></div><div class="comparison-section"><h3 style="color:#52B788;margin-bottom:10px">Action Plan</h3><div style="background:#2a2a2a;padding:15px;border-radius:8px;border:1px solid #52B788"><div style="color:#52B788;font-weight:500;margin-bottom:10px;font-size:0.95em">Step-by-Step Instructions:</div><div style="color:#fff;line-height:2;font-size:0.95em">${{p.trades.split(';').map((t,i)=>`<div style="padding:5px 0;border-bottom:1px solid #333">${{i+1}}. ${{t.trim()}}</div>`).join('')}}</div><div style="margin-top:15px;padding-top:15px;border-top:1px solid #333;color:#aaa;font-size:0.85em"><strong style="color:#52B788">Budget Required:</strong> 10% of portfolio value<br><strong style="color:#52B788">Complexity:</strong> ${{opNum}} transaction${{opNum>1?'s':''}}</div></div></div>`;document.getElementById('comparisonPanel').classList.add('active')}}
